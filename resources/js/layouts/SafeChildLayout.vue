...
const page = usePage();
const notif = computed<any>(
    () => (page.props as any).notif ?? { unread: 0, latest: null },
);
const unread = computed<number>(() => notif.value.unread ?? 0);
const showNotificationToast = computed(
    () => page.component !== 'safechild/MainAdmin',
);

// Active user shown in the navbar. Per-page can override via `currentUser` prop.
const DEFAULT_USER = {
    name: 'Olena Franko',
...
const currentUser = computed<{
    ...
    photo?: string | null;
    initials?: string;
}>(() => (page.props as any).currentUser ?? DEFAULT_USER);
...
const toast = ref<any>(showNotificationToast.value ? notif.value.latest : null);
let seenId: number | null = notif.value.latest?.id ?? null;

watch(
    () => notif.value.latest?.id,
    (id) => {
        if (showNotificationToast.value && id && id !== seenId) {
...
<template>
    <div class="min-h-screen bg-[#eef1f1] text-neutral-900">
        ...
        >
            <div
                class="flex h-[72px] items-center justify-between gap-3 px-6 sm:px-10"
            >
                <Link
                    href="/"
                ...
                <div class="flex shrink-0 items-center gap-3 sm:gap-4">
                    ...
                    >
                        ...
                        <span
                            v-if="unread > 0"
                            class="absolute top-2.5 right-2.5 size-2 rounded-full bg-red-500 ring-2 ring-white"
                        />
                    </Link>
                    <button class="flex cursor-pointer items-center gap-2.5">
                        ...
                        <ChevronDown
                            class="hidden size-4 text-neutral-400 md:inline"
                        />
                    </button>
                </div>
            </div>
        ...
        <Teleport to="body">
            ...
            >
                <div
                    v-if="showNotificationToast && toast"
                    class="fixed right-6 bottom-6 z-50 w-80 rounded-2xl border border-red-200 bg-red-50 p-4 shadow-xl"
                >
                    <div class="flex items-start justify-between">
                        <span
                            class="rounded-full bg-red-500 px-2.5 py-1 text-xs font-semibold text-white"
                            >CHECK IT OUT</span
                        >
                        <button
                            @click="toast = null"
                    ...
                    <p class="mt-1 text-xs text-neutral-500">
                        New risk factors recorded. Review the case and assess
                        next steps.
                    </p>
                    <Link
                        ...
                        >More details <ChevronRight class="size-4"
                    /></Link>
                </div>
            </Transition>
        </Teleport>
