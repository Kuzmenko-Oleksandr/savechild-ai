...
function goPage(page: number) {
    router.get(
        '/notifications',
        { status: props.tab === 'all' ? null : props.tab, page },
        { preserveScroll: true, preserveState: true, replace: true },
    );
}
</script>
...
<template>
    ...
    <div class="mt-5 space-y-4">
        ...
        >
            ...
            <p
                class="mt-1 line-clamp-3 text-sm leading-relaxed text-neutral-500"
            >
                {{ n.body }}
            </p>
            <Link
    ...
    <p
        v-if="!items.length"
        class="mt-5 rounded-2xl bg-white py-12 text-center text-sm text-neutral-400"
    >
        No notifications.
    </p>
